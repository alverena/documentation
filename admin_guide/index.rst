
Administrator Guide
====================

The guide is dedicated to system administration and configuration. It describes tools for system management and administration, aimed to be used primarily by the system 
administrators.

Please note that in many cases, the tools may be not available to the usual users, subject to the permissions defined.


  * :doc:`/admin_guide/configuration/system_config`
  
     * :doc:`/admin_guide/configuration/system_mailboxes`
      
  
  * :doc:`/admin_guide/entities/entities`
 
     * :doc:`/admin_guide/entities/entity_fields` 
  
 
        
  * :doc:`/admin_guide/user_management/index`

     * :doc:`/admin_guide/user_management/user_management_overview`
        
     * :doc:`/admin_guide/user_management/user_management_organization`
    
       * :doc:`/admin_guide/user_management/multiple_organizations_support`
      
      
     * :doc:`/admin_guide/user_management/user_management_bu`
      
     * :doc:`/admin_guide/user_management/user_management_groups`      

     * :doc:`/admin_guide/user_management/user_management_users`



  * :doc:`/admin_guide/access_roles_management/index`
   
     * :doc:`/admin_guide/access_roles_management/access_management_roles`  
        
       * :doc:`/admin_guide/access_roles_management/access_management_roles_interface`  
       
       * :doc:`/admin_guide/access_roles_management/access_management_roles_actions`
      
       * :doc:`/admin_guide/access_roles_management/admin_capabilities`  
       
    
     * :doc:`/admin_guide/access_roles_management/access_management_user_access_settings`  

     * :doc:`/admin_guide/access_roles_management/access_management_ownership_type`  
       
     * :doc:`/admin_guide/access_roles_management/access_management_access_levels`
      
     * :doc:`/admin_guide/access_roles_management/access_management_examples`    
    

  * :doc:`/admin_guide/other_tools/rfm_admin`
  
  * :doc:`/admin_guide/integrations/index`
      
    * :doc:`/admin_guide/integrations/integrations_overview`
    * :doc:`/admin_guide/integrations/google_single_sign_on`
    * :doc:`/admin_guide/integrations/magento_channel_integration`
    * :doc:`/admin_guide/integrations/ldap_integration`
    * :doc:`/admin_guide/integrations/mailchimp_integration`
    * :doc:`/admin_guide/integrations/sending_email_campaign_via_mailchimp`
    * :doc:`/admin_guide/integrations/dotmailer_integration`
    * :doc:`/admin_guide/integrations/sending_email_campaign_via_dotmailer`
    * :doc:`/admin_guide/integrations/ms_exchange_integration`
    * :doc:`/admin_guide/integrations/outlook_sync`
    * :doc:`/admin_guide/integrations/embedded_forms`
    * :doc:`/admin_guide/integrations/hangouts`  
    * :doc:`/admin_guide/integrations/zendesk_integration`

    
  * :doc:`/admin_guide/system/processes`
  * :doc:`/admin_guide/system/package_manager`  
    



.. toctree::
    :hidden:
    :maxdepth: 2
    :glob:

    access_roles_management/index
    configuration/index
    entities/index
    integrations/index
    other_tools/index
    system/index
    user_management/index



